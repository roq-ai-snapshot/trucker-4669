...
import { getPositions, deletePositionById } from 'apiSdk/positions';
import { PositionInterface } from 'interfaces/position';

type ColumnType = ColumnDef<PositionInterface, unknown>;

interface PositionListPageProps {
...
export function PositionListPage(props: PositionListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PositionInterface>>(
    () => `/positions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PositionInterface) => {
    if (hasAccess('position', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/positions/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/positions/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('position', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/positions/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('position', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="position" />
        </Flex>

        {hasAccess('position', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/positions/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
